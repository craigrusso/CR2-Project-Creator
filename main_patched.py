...
def main():
    ...
    # Set application details (name, version) for system integration
    app = QApplication(sys.argv)
    app.setApplicationName(APP_NAME)
    
    # Ensure APP_BUILD_NUMBER is defined; if not, provide a fallback or handle error
    # For now, assume APP_BUILD_NUMBER is correctly imported from app_config
    app_version_for_id = APP_VERSION_NUMBER.replace('.', '_')
    ...
    if platform.system() == "Windows":
        try:
            ...
            print(f"Windows AppUserModelID set to: {myappid}") # Using print for patched version
        except Exception as e:
            print(f"Failed to set Windows AppUserModelID: {e}")

    app.setApplicationVersion(APP_VERSION_NUMBER) # Changed APP_VERSION
