...
impl BlastEngine {
    ...
    async fn prepare_blast_operation(&self, source_files: &[PathBuf], job_id: &str) -> Result<()> {
        println!("BLAST Phase 1: Preparing operation for {} files", source_files.len());
        ...
        for source_file in source_files {
            ...
            let relative_path = source_file.file_name()
        ...
        *self.file_manifest.write() = manifest;
        ...
        if let Some(ref event_hub) = self.event_hub {
            let _ = event_hub.emit_event(TransferEvent::new(
                ...
                EventPayload::Blast {
                    ...
                    distribution_destinations: self.config.distribution_targets.iter()
        ...
        println!("BLAST preparation complete: {} files, {:.2} GB total", 
            source_files.len(), 
    ...
    async fn execute_cache_load(&self, job_id: &str) -> Result<CacheLoadResult> {
        ...
        for (index, file_record) in manifest.iter().enumerate() {
            ...
            self.copy_to_cache(&file_record.source_path, &file_record.cache_path, chunk_size).await?;
            ...
            // Update statistics
            self.stats.cached_bytes.store(bytes_cached, Ordering::Relaxed);
            self.stats.files_processed.store(files_cached as u64, Ordering::Relaxed);
            ...
            self.stats.cache_speed_mbps.store(current_speed as u64, Ordering::Relaxed);
        ...
        println!("BLAST cache load complete: {} files, {:.2} GB @ {:.1} MB/s average", 
            files_cached,
    ...
    async fn execute_distribution(&self, job_id: &str) -> Result<DistributionResult> {
        ...
        println!("BLAST distribution complete: {} files to {} destinations @ {:.1} MB/s average",
    ...
    async fn copy_to_cache(&self, source: &Path, cache_dest: &Path, chunk_size: usize) -> Result<()> {
    ...
    async fn distribution_worker(
        ...
        while let Ok(task) = receiver.recv() {
            ...
            stats.distributed_bytes.fetch_add(task.file_size, Ordering::Relaxed);
        }
    ...
    async fn set_phase(&self, phase: BlastPhase, job_id: &str) -> Result<()> {
        ...
        if let Some(previous_phase) = self.stats.phase_timings.read().keys().last().copied() {
            if let Some(previous_start) = self.stats.phase_timings.read().get(&previous_phase) {
                let duration = start_time.duration_since(
                    SystemTime::now() - *previous_start
                ).unwrap_or_default();
                self.stats.phase_timings.write().insert(previous_phase, duration);
            }
        }
        
        // Set new phase
        *self.stats.current_phase.write() = phase;
        self.stats.phase_timings.write().insert(phase, Duration::from_secs(0));
    ...
    fn get_final_stats(&self) -> BlastStatsSummary {
        BlastStatsSummary {
            ...
            distribution_speed_mbps: self.stats.distribution_speed_mbps.load(Ordering::Relaxed) as f64,
...
mod tests {
    ...
    async fn test_blast_preparation() {
        ...
        fs::write(&test_file, b"Hello BLAST!").await.expect("Failed to write test file");
        ...
        let result = engine.prepare_blast_operation(&[test_file], "test_job").await;
        assert!(result.is_ok());
