...
/// Worker that writes files to a single destination
pub(crate) struct DestinationWorker {
    ...
    memory_tracker: Option<Arc<AtomicUsize>>,
}
...
impl DestinationWorker {
    ...
    pub fn run(self) {
        ...
        while let Ok(command) = self.commands.recv() {
            ...
            match command {
                ...
                WorkerCommand::Chunk { data } => {
                    ...
                    // Write at THIS destination's speed (independent of others)
                    if let Some(active) = state.as_mut() {
                        active.write(data.as_ref());
                    }

                    // Signal producer: chunk consumed, memory can be freed
        ...
        // Cleanup any remaining active file
