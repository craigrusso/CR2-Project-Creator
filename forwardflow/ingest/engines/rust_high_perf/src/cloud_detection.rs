...
impl CloudProvider {
    ...
    pub fn get_sync_folder_names(&self) -> Vec<String> {
        match self {
            ...
            CloudProvider::Dropbox => vec!["Dropbox".to_string()],
            CloudProvider::GoogleDrive => vec!["Google Drive".to_string(), "My Drive".to_string()],
            CloudProvider::ICloud => vec!["iCloud Drive".to_string(), "iCloud".to_string()],
            CloudProvider::Box => vec!["Box".to_string()],
            CloudProvider::Mega => vec!["MEGA".to_string()],
            CloudProvider::PCloud => vec!["pCloud Drive".to_string()],
            CloudProvider::SyncCom => vec!["Sync".to_string()],
            CloudProvider::Tresorit => vec!["Tresorit".to_string()],
            CloudProvider::SpiderOak => vec!["SpiderOak".to_string()],
...
impl CloudDetectionManager {
    ...
    fn initialize_patterns(&mut self) {
        if let Ok(mut patterns) = self.detection_patterns.lock() {
            ...
            patterns.insert(
                "dropbox".to_string(),
                vec!["dropbox".to_string(), "dropbox.exe".to_string()],
            );
            ...
            patterns.insert(
                "box".to_string(),
                vec!["box".to_string(), "box sync".to_string()],
            );

            patterns.insert(
                "mega".to_string(),
                vec!["mega".to_string(), "megasync".to_string()],
            );

            patterns.insert(
                "pcloud".to_string(),
                vec!["pcloud".to_string(), "pcloud drive".to_string()],
            );

            patterns.insert(
                "sync_com".to_string(),
                vec!["sync.com".to_string(), "sync".to_string()],
            );

            patterns.insert("tresorit".to_string(), vec!["tresorit".to_string()]);

            patterns.insert("spideroak".to_string(), vec!["spideroak".to_string()]);
        }
    }
                    ...
                    if path_str.contains(pattern) {
                        ...
                        if provider != CloudProvider::Unknown {
                            ...
                            if let Ok(mut locations) = self.known_locations.lock() {
                                locations
                                    .insert(path.to_string_lossy().to_string(), location.clone());
                            }
    ...
    pub fn get_cloud_provider(&self, path: &Path) -> Option<CloudProvider> {
        self.detect_cloud_storage(path)
            ...
            match location.provider {
                CloudProvider::OneDrive => {
                    recommendations.push(
                        "Consider using OneDrive's built-in sync for better performance"
                            .to_string(),
                    );
                    recommendations
                        .push("Monitor OneDrive sync status to avoid conflicts".to_string());
                }
                CloudProvider::Dropbox => {
                    recommendations
                        .push("Dropbox provides good sync performance for large files".to_string());
                    recommendations.push("Use selective sync for better control".to_string());
                }
                CloudProvider::GoogleDrive => {
                    recommendations.push(
                        "Google Drive works well with Google Workspace integration".to_string(),
                    );
                    recommendations.push(
                        "Consider using Google Drive File Stream for large files".to_string(),
                    );
                }
                CloudProvider::ICloud => {
                    recommendations
                        .push("iCloud Drive integrates well with macOS and iOS".to_string());
                    recommendations.push("Monitor iCloud storage space".to_string());
                }
                _ => {
                    recommendations
                        .push("Monitor sync status to ensure data consistency".to_string());
                    recommendations
                        .push("Consider backup strategies for cloud storage".to_string());
                }
            }
        }
