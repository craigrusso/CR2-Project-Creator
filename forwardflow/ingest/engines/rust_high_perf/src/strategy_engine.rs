...
use crate::event_hub_v2::{EventHub, TransferEvent, EventType, EventPriority, EventPayload};
use pyo3::prelude::*;
...
impl DestinationType {
    ...
    pub fn supports_multipart(&self) -> bool {
        matches!(self, DestinationType::NetworkShare | DestinationType::CloudStorage)
    }
    
    /// Whether this destination can be used as a BLAST cache
    pub fn suitable_for_blast(&self) -> bool {
        matches!(self, DestinationType::NvmeSsd | DestinationType::LocalSsd | DestinationType::GpuMemory)
    }
...
impl TransferStrategy {
    ...
    pub fn estimate_transfer_time(&self, total_bytes: u64, analyses: &[DestinationAnalysis]) -> Duration {
        match self {
            TransferStrategy::DirectCopy { destination, .. } => {
                if let Some(analysis) = analyses.iter().find(|a| a.path == *destination) {
                    let seconds = total_bytes as f64 / (analysis.benchmarked_speed_mbps * 1024.0 * 1024.0);
            ...
            TransferStrategy::MemoryStaging { destinations, .. } => {
                ...
                let write_times: Vec<f64> = destinations.iter()
                    .filter_map(|dest| {
                        analyses.iter().find(|a| a.path == *dest)
                            .map(|a| total_bytes as f64 / (a.benchmarked_speed_mbps * 1024.0 * 1024.0))
                    })
                    .collect();
            ...
            TransferStrategy::BlastWorkflow { blast_cache, .. } => {
                ...
                if let Some(cache_analysis) = analyses.iter().find(|a| a.path == *blast_cache) {
                    let seconds = total_bytes as f64 / (cache_analysis.benchmarked_speed_mbps * 1024.0 * 1024.0);
                    Duration::from_secs_f64(seconds)
                } else {
...
impl TransferStrategyEngine {
    ...
    pub fn analyze_and_select_strategy(
        ...
        println!("Analyzing {} destinations for optimal transfer strategy", destinations.len());
        ...
        let analyses: Result<Vec<_>, _> = destinations.par_iter()
        ...
        println!("Selected strategy: {} (estimated time: {:?})", 
            strategy.name(),
            estimated_time
        );
        ...
        if let Some(ref event_hub) = self.event_hub {
            let _ = event_hub.emit_event(TransferEvent::new(
                ...
                EventPayload::Job {
                    ...
                    peak_speed_mbps: analyses.iter().map(|a| a.benchmarked_speed_mbps).fold(0.0, f64::max),
                    progress_percent: 0.0,
    ...
    pub fn analyze_destination(&self, destination: &Path) -> Result<DestinationAnalysis> {
        ...
        self.analysis_cache.write().insert(destination.to_path_buf(), analysis.clone());
        ...
        if let Some(blast_path) = blast_cache {
            if let Some(blast_analysis) = analyses.iter().find(|a| a.path == blast_path) {
                if blast_analysis.dest_type.suitable_for_blast() {
                    let other_destinations: Vec<PathBuf> = analyses.iter()
                    ...
                    if !other_destinations.is_empty() {
                        return Ok(TransferStrategy::BlastWorkflow {
                            ...
                            distribution_strategy: Box::new(self.select_multi_destination_strategy(&other_destinations, analyses, job_id)?),
                        });
                    }
                }
            }
    ...
    fn select_multi_destination_strategy(
        ...
        let total_speed: f64 = analyses.iter()
        ...
        if speed_variance < 2.0 { // Destinations have similar speeds
    ...
    fn calculate_optimal_chunk_size(&self, dest_type: DestinationType, volume_info: &VolumeInfo) -> usize {
    ...
    fn should_use_gpu_acceleration(
        ...
        let has_fast_destinations = analyses.iter()
...
mod tests {
    ...
    fn test_gpu_acceleration_strategy() {
        ...
        assert!(matches!(strategy.name(), "MemoryStaging" | "HybridMultiStrategy" | "GpuAccelerated"));
    }
}