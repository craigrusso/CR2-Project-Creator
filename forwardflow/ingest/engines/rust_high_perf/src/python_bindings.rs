...
use crate::event_hub_v2::{EventHub, TransferEvent, EventType, EventPriority, EventPayload, EventHubStats};
use crate::strategy_engine::{TransferStrategyEngine, TransferStrategy, DestinationAnalysis};
use crate::blast_engine::{BlastEngine, BlastConfig, BlastTransferResult};
...
impl PyEventHubV2 {
    ...
    fn new() -> PyResult<Self> {
        match EventHub::new() {
            ...
            Err(e) => Err(PyRuntimeError::new_err(format!("Failed to create EventHub: {}", e))),
        }
    }
...
impl PyEventHubV2 {
    ...
    fn dict_to_transfer_event(&self, _py: Python, data: &PyDict) -> PyResult<TransferEvent> {
        ...
        let event_type = match event_type_str.as_str() {
            ...
            _ => return Err(PyValueError::new_err(format!("Unknown event type: {}", event_type_str))),
        };
        ...
        match payload_type.as_str() {
            ...
            "Job" => {
                Ok(EventPayload::Job {
                    ...
                    completed_destinations: Self::get_u64_item(data, "completed_destinations", 0)? as usize,
...
impl PyTransferStrategyEngine {
    ...
    fn analyze_and_select_strategy(
        ...
        match result {
            ...
            Err(e) => Err(PyRuntimeError::new_err(format!("Strategy analysis failed: {}", e))),
        }
    }
    ...
    fn analyze_destination(&self, py: Python, destination: &str) -> PyResult<PyObject> {
        ...
        match self.inner.analyze_destination(&dest_path) {
            ...
            Err(e) => Err(PyRuntimeError::new_err(format!("Destination analysis failed: {}", e))),
        }
    }
}
...
impl PyTransferStrategyEngine {
    ...
    fn strategy_to_dict(&self, py: Python, strategy: &TransferStrategy) -> PyResult<PyObject> {
        ...
        match strategy {
            TransferStrategy::DirectCopy { destination, use_memory_mapping, chunk_size_mb } => {
            ...
            TransferStrategy::MemoryStaging { destinations, buffer_size_mb, parallel_writes } => {
                dict.set_item("type", "MemoryStaging")?;
                let dest_list = PyList::new(py, destinations.iter().map(|p| p.to_string_lossy().to_string()));
            ...
            TransferStrategy::GpuAccelerated { gpu_staging, final_destinations, compute_pipeline, use_unified_memory } => {
                dict.set_item("type", "GpuAccelerated")?;
                dict.set_item("gpu_staging", gpu_staging.to_string_lossy().as_ref())?;
                let dest_list = PyList::new(py, final_destinations.iter().map(|p| p.to_string_lossy().to_string()));
            ...
            TransferStrategy::BlastWorkflow { blast_cache, final_destinations, .. } => {
    ...
    fn analysis_to_dict(&self, py: Python, analysis: &DestinationAnalysis) -> PyResult<PyObject> {
        ...
        volume_dict.set_item("mount_point", analysis.volume_info.mount_point.to_string_lossy().as_ref())?;
        volume_dict.set_item("is_case_sensitive", analysis.volume_info.is_case_sensitive)?;
        volume_dict.set_item("supports_sparse_files", analysis.volume_info.supports_sparse_files)?;
...
impl PyBlastEngine {
    ...
    fn blast_result_to_dict_impl(py: Python, result: &BlastTransferResult) -> PyResult<PyObject> {
        ...
        cache_dict.set_item("duration_seconds", result.cache_result.duration.as_secs_f64())?;
        ...
        dist_dict.set_item("files_distributed", result.distribution_result.files_distributed)?;
        dist_dict.set_item("bytes_distributed", result.distribution_result.bytes_distributed)?;
        dist_dict.set_item("duration_seconds", result.distribution_result.duration.as_secs_f64())?;
        dist_dict.set_item("average_speed_mbps", result.distribution_result.average_speed_mbps)?;
        ...
        verify_dict.set_item("verification_passed", result.verification_result.verification_passed)?;
        verify_dict.set_item("failed_files", result.verification_result.failed_files.len())?;
