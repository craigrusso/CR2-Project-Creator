...
impl TransferEvent {
    pub fn new(event_type: EventType, priority: EventPriority, job_id: String, payload: EventPayload) -> Self {
        Self {
            event_type,
            priority,
...
impl EventStats {
    pub fn record_event(&self, event_type: EventType, processing_time_ns: u64) {
        ...
        if processing_time_ns > current_peak {
            self.peak_processing_time_ns.store(processing_time_ns, Ordering::Relaxed);
        }
...
impl EventHub {
    ...
    pub fn new() -> Result<Self> {
        ...
        println!("EventHub V2.0 initialized with {} handler threads", handler_threads.len());
    ...
    pub fn emit_event(&self, event: TransferEvent) -> Result<()> {
        let sender = match event.event_type {
            EventType::FileStarted | EventType::FileProgress | 
            EventType::FileCompleted | EventType::FileError | 
            EventType::FileHashCalculated => &self.file_sender,
            
            EventType::DestStarted | EventType::DestProgress | 
            EventType::DestCompleted | EventType::DestError => &self.dest_sender,
            
            EventType::JobStarted | EventType::JobProgress | 
            EventType::JobCompleted | EventType::JobCancelled | 
            EventType::JobError | EventType::JobStrategySelected => &self.job_sender,
            
            EventType::BlastInitiated | EventType::BlastCompleted | 
            EventType::BlastDistributionStarted | EventType::BlastDistributionCompleted => &self.blast_sender,
    ...
    pub fn get_stats(&self) -> EventHubStats {
        EventHubStats {
            ...
            events_by_type: self.stats.events_by_type.iter()
...
mod tests {
    ...
    fn test_event_routing() {
        ...
        hub.emit_event(file_event).expect("Failed to emit file event");
