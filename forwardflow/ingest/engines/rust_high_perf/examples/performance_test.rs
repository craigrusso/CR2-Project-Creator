...
use rust_high_perf_engine::blast_engine::{BlastConfig, BlastEngine};
use rust_high_perf_engine::event_hub_v2::{
    EventHub, EventPayload, EventPriority, EventType, TransferEvent,
};
use rust_high_perf_engine::strategy_engine::{DestinationType, TransferStrategyEngine};
...
async fn test_event_hub_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  ✓ Event throughput: {:.0} events/second",
        events_per_second
    );
    ...
    match result {
        Ok((strategy, analyses)) => {
            ...
            if let Some(ref gpu_info) = analyses[0].gpu_info {
                println!(
                    "  ✓ GPU detected: {} ({:.1} GB)",
                    gpu_info.device_name, gpu_info.memory_gb
                );
            } else {
...
async fn test_blast_engine() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = blast_engine
        .execute_blast_transfer(&test_files, "blast_perf_test")
        .await;
...
async fn test_threading_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "    - {} threads × {} iterations",
        cpu_count, iterations_per_thread
    );
    println!(
        "    - {} total operations in {:?}",
    ...
    println!(
        "    - {} async tasks completed in {:?}",
        task_count, async_duration
    );
    println!(
        "    - {:.1} tasks/second",
        task_count as f64 / async_duration.as_secs_f64()
    );
