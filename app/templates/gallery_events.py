...
class GalleryEvents:
    ...
    @staticmethod
    def on_toggle_view_action(gallery, action_name):
        """Toggle a view action in the view menu"""
        # Get all view actions
        view_actions = {
            "card": gallery.card_view_action,
            "list": gallery.list_view_action,
            "table": gallery.table_view_action
        }
        
        # Uncheck all actions
        for action_key, action in view_actions.items():
            action.setChecked(False)
        
        # Check the selected action
        if action_name in view_actions:
            view_actions[action_name].setChecked(True)
    
    @staticmethod
    def on_filter_templates(gallery):
        """Filter templates based on search text and category"""
        # Get the search text and category
        search_text = gallery.search_box.text() if hasattr(gallery, "search_box") else ""
        
        category_text = "All"
        if hasattr(gallery, "category_filter") and gallery.category_filter.currentText():
            category_text = gallery.category_filter.currentText()
        
        # Filter templates
        if hasattr(gallery, "_populate_templates"):
            # Filter using the search text and category
            gallery._populate_templates(
                search_term=search_text,
                category=category_text if category_text != "All" else None
            )
