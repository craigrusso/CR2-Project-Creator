    ...
    def __init__(self, parent=None, folder_name="", app=None):
        ...
        try:
            ...
            if platform.system() == "Windows":
                # On Windows, use the native folder icon without modifications
                app = QApplication.instance()
                if not app:
                    print("ERROR (FolderCard): QApplication not initialized")
                    self.icon_label.setText("ERROR")
                else:
                    # Get icon from system style
                    style = app.style()
                    icon = style.standardIcon(QStyle.StandardPixmap.SP_DirIcon) 
                    
                    # Debug output
                    print(f"DEBUG (FolderCard): Windows folder icon retrieved, null: {icon.isNull()}")
                    ...
                    if not pixmap.isNull():
                        ...
                        print("ERROR (FolderCard): Failed to get Windows folder icon pixmap")
                        # Fallback to another icon
                        fallback_icon = style.standardIcon(QStyle.StandardPixmap.SP_FileDialogNewFolder)
                        fallback_pixmap = fallback_icon.pixmap(64, 64)
                        if not fallback_pixmap.isNull():
                            self.icon_label.setPixmap(fallback_pixmap)
                        else:
                            self.icon_label.setText("FOLDER")
            else:
                # On macOS, use existing code
    ...
    def resize_icon(self, scale_percent):
        ...
        try:
            ...
            if platform.system() == "Windows":
                ...
                app = QApplication.instance()
                if not app:
                    print(f"ERROR (resize_icon): QApplication not initialized")
                    self.icon_label.setText("ERROR")
                else:
                    style = app.style()
                    icon = style.standardIcon(QStyle.StandardPixmap.SP_DirIcon)
                    ...
                    # Debug output
                    print(f"DEBUG (resize_icon): Windows folder icon retrieved, size: {new_icon_size}, null: {icon.isNull()}")
                    
                    pixmap = icon.pixmap(new_icon_size, new_icon_size)
                    if not pixmap.isNull():
                        self.icon_label.setPixmap(pixmap)
                    else:
                        print(f"ERROR (resize_icon): Failed to get Windows folder icon pixmap at size {new_icon_size}")
                        # Fallback to another icon
                        fallback_icon = style.standardIcon(QStyle.StandardPixmap.SP_FileDialogNewFolder)
                        fallback_pixmap = fallback_icon.pixmap(new_icon_size, new_icon_size)
                        if not fallback_pixmap.isNull():
                            self.icon_label.setPixmap(fallback_pixmap)
                        else:
                            self.icon_label.setText("FOLDER")
