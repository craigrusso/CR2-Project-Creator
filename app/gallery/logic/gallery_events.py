...
class GalleryEvents:
    ...
    @staticmethod
    def on_template_select(gallery, template_data):
        """Handle template selection, now using GallerySelectionManager."""
        ...
        # 2. Any existing multi-selection should be cleared.
        gallery.selection_manager.clear_multi_selection_list(emit_signal=False) # Clear multi, don't signal yet
        gallery.selection_manager.set_primary_selection(template_data, emit_signal=True) # Set primary, this will signal
    ...
    def mouse_press_event(gallery, event):
        ...
        if hasattr(gallery, 'selection_manager'):
            ...
            if current_primary or current_multi:
                gallery.selection_manager.clear_all_selections(emit_signal=True) # This will trigger UI update via signal
