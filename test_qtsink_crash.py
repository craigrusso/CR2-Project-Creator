#!/usr/bin/env python3
"""
Test to isolate the QtSink crash issue
"""
...
def test_qtsink_crash():
    """Test if the issue is with QtSink"""
    print("Testing QtSink crash issue...")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Create a simple mock sink to test if the issue is with QtSink
            class MockSink:
                def emit(self, event_type, payload):
                    print(f"MOCK SINK: Received event {event_type} with payload {payload}")
            
            # Test the Python copy engine with a mock sink
            engine = PythonCopyEngine(sink=MockSink())
            ...
            print("\n✓ All tests passed! The issue is with QtSink, not the engine.")
            return True
