...
class IconNameDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        if option.widget:
            ...
            if hasattr(view, 'selectionModel'):
                ...
                if selection_model:
                    ...
                    is_selected_model_check = selection_model.isSelected(index)
                    print(f"[DEBUG IconNameDelegate.paint] Item: '{item_text_for_log}', selection_model.isSelected(index): {is_selected_model_check}")
                    is_selected = is_selected_model_check # Use this as the primary check
                    
        # 2. Also check the option state as fallback OR for confirmation
        is_selected_option_state = bool(option.state & QStyle.State_Selected)
        print(f"[DEBUG IconNameDelegate.paint] Item: '{item_text_for_log}', option.state & QStyle.State_Selected: {is_selected_option_state}")
        
        # Decide final is_selected (prioritize model check if available, otherwise option state)
        if not option.widget or not hasattr(option.widget, 'selectionModel') or not option.widget.selectionModel():
            is_selected = is_selected_option_state # Fallback if model check wasn't possible
        ...
        if is_selected:
            ...
            option.state &= ~QStyle.State_Selected # Ensure it's not set if not selected
        # <<< END NEW >>>
        ...
        if is_warning:
            ...
            painter.drawText(char_rect, Qt.AlignmentFlagFlagFlag.AlignCenter, "⚠")
            painter.restore()
        ...
        if text:
            ...
            painter.drawText(text_rect, Qt.AlignmentFlagFlagFlag.AlignVCenter | Qt.AlignmentFlagFlagFlag.AlignLeft, elided_text)
...
class SortableHeaderView(QHeaderView):
    ...
    def paintSection(self, painter, rect, logicalIndex):
        ...
        alignment = Qt.AlignmentFlagFlagFlag.AlignLeft | Qt.AlignmentFlagFlagFlag.AlignVCenter # Consistent alignment
        ...
        if is_sorted_section:
            ...
            if platform.system() == "Windows":
                ...
                painter.drawText(arrow_rect, Qt.AlignmentFlagFlagFlag.AlignCenter, indicator_char)
            else:
...
class TemplateTableView(QTableView):
    ...
    def _configure_header(self):
        ...
        header.setDefaultAlignment(Qt.AlignmentFlagFlagFlag.AlignLeft | Qt.AlignmentFlagFlagFlag.AlignVCenter)
