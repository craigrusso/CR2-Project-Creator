...
class AcceptanceCriteriaValidator:
    ...
    def test_job_aggregator_progress_tracking(self) -> bool:
        ...
        try:
            ...
            # Pre-generate deterministic file sizes so we can supply manifest totals
            file_sizes = [
                random.randint(1024 * 1024, 16 * 1024 * 1024)  # 1 MiB to 16 MiB for stable testing
                for _ in range(total_files)
            ]
            dataset_total_bytes = sum(file_sizes)

            # Create JobAggregator with known dataset total for accurate denominators
            aggregator = JobAggregator(total_files, destinations, dataset_total_bytes=dataset_total_bytes)
            ...
            results = {
                ...
                'total_files_correct': final_snapshot.job_total_files == total_files * len(destinations),
                'completed_files_correct': final_snapshot.job_completed_files == total_files * len(destinations),
            ...
            print(f"    Total files: {'✅' if results['total_files_correct'] else '❌'} ({final_snapshot.job_total_files}/{total_files * len(destinations)})")
            print(f"    Completed files: {'✅' if results['completed_files_correct'] else '❌'} ({final_snapshot.job_completed_files}/{total_files * len(destinations)})")
